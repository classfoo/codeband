...
impl CodingToolDriver for QoderCliDriver {
    ...
    fn check_installed(&self, config: &Value) -> anyhow::Result<bool> {
        ...
        let command = config.get("command").and_then(Value::as_str).unwrap_or("qoder");
        Ok(Command::new("sh")
            .arg("-c")
    ...
    fn install_tool(&self, config: &Value) -> anyhow::Result<()> {
        ...
        let status = Command::new("sh")
            .arg("-c")
            .arg("npm install -g @qoder/cli")
            .status()?;
        if !status.success() {
            anyhow::bail!("failed to install Qoder CLI");
        }
        if !self.check_installed(config)? {
            anyhow::bail!("qoder command still not found after installation");
        }
        Ok(())
    }
    ...
    fn start_tool(&self, config: &Value) -> anyhow::Result<()> {
        ...
        match status {
            ...
            Ok(_) => anyhow::bail!("qoder command failed to start"),
            Err(err) => anyhow::bail!("failed to execute qoder command: {err}"),
        }
    }
